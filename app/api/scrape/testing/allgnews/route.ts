...
export async function GET() {
  ...
  console.time('[2] fetchesData');
  const fetchesData = settledResults
    .map((result, i) => {
  ...
  console.log('fetchesData: ', fetchesData);
  console.timeEnd('[2] fetchesData');
  ...
  console.time('articlesPromises');
  const articlesPromises = $('article.IBr9hb, article.IFHyqb.DeXSAc')
    .filter((_, article) => {
    ...
    .map(async (i, article) => {
      try {
        ...
        return { status: 'fulfilled', value: articleObj };
      } catch (error) {
        return { status: 'rejected', reason: error };
      }
    })
    .get();
  console.timeEnd('articlesPromises');
