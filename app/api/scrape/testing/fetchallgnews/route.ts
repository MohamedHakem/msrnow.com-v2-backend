...
export async function GET() {
  ...
  const promises = await Promise.allSettled(
    urls.map(async (url, i) => {
      // console.time(`${i} fetch resolved`);
      try {
        // console.log(`${i} fetch started`);
        const data = await fetch(url).then((res) => res.text());
        // data ? console.timeEnd(`${i} fetch resolved`) : null;
        return { url, data };
      } catch (error) {
  ...
  return NextResponse.json({ status: 200, fetchDataArray });
}
